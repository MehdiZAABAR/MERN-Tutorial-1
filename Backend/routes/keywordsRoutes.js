...
import {Keyword, ValidateKeyword} from "../models/all_collections_models.js"
...
router.get( '/', async ( request, response) => {
    try {
        const mRecord = await Keyword.find( {});
...
router.get( '/:id', async ( request, response) => {
    try {
        ...
        const mRecord = await Keyword.findById( id);
        if( !mRecord)
...
router.put( '/:id', async ( request, response) => {
    try {
        ...
        const mRecord = await Keyword.findByIdAndUpdate( id, request.body);
        if( !mRecord)
            return response.status(404).json( {message: 'id not found'});
