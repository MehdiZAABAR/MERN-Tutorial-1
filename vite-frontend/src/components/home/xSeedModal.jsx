...
const SeedModal = ({ seed, onClose }) => {
    return (
        <div className='fixed bg-black bg-opacity-60 top-0 left-0 right-0 bottom-0 z-50 flex justify-center items-center' onClick={onClose}>
            <div onClick={(event) => { event.stopPropagation() }} className='w-[600px] max-w-full height-[400px] bg-white rounded-xl p-4 flex flex-col relative'>
                <AiOutlineClose className='absolute right-6 top-6 text-3xl text-red-600 cursor-pointer' onClick={onClose} />
                <h2 className='w-fit px-4 py-1 bg-red-300 rounded-lg '>{seed.easyName}</h2>
                ...
                <div className='flex justify-start items-center gap-x-2'>
                    ...
                    <p className="mt-4">Anything you need to display!</p>
